...
public class ChatVM extends ViewModel {
    ...
    /**
     * Configura la sala privada. Se llama al abrir el chat.
     */
    public void setupChat(String myUid, String otherUid) {
        if (currentChatId != null) return;

        // 1. Crear ID único ordenado alfabéticamente
        String[] ids = {myUid, otherUid};
        Arrays.sort(ids);
        currentChatId = "chat_" + ids[0] + "_" + ids[1];

        Log.d(TAG, "Sala Privada: " + currentChatId);

        // 2. Cargar historial antiguo de esta sala
        loadHistoryFromFirebase();

        // 3. Conectar al canal MQTT de esta sala
        connectAndSubscribe();
    }
    ...
    public void sendMessage(String sender, String content) {
        ...
        try {
            ...
            // Publicar en tópico privado
            mqttRepo.publish(currentChatId, json.toString());

            // Guardar en base de datos privada
